var bufferview = $.extend({}, $.fn.datagrid.defaults.view, {
	render: function(target, container, frozen){
		...
		for(var i=0; i<rows.length; i++) {
			...
			contentTable.push('<tr id="' + rowId + '" datagrid-row-index="' + index + '" ' + cls + ' ' + style + '>');
			contentTable.push(this.renderRow.call(this, target, fields, frozen, index, rows[i]));
			contentTable.push('</tr>');
			index++;
		}		

		//create new table if not existed, else then append
		if ($(target).datagrid('getPanel').find('.datagrid-btable').length === 0) {
			var table = ['<table class="datagrid-btable" cellspacing="0" cellpadding="0" border="0"><tbody>'];
			table.push(contentTable.join(''));
			table.push('</tbody></table>');
			
			$(container).append(table.join(''));
		} else {
			$(container).find('.datagrid-btable tbody').append(contentTable.join(''));
		}
		
		var state = $.data(target, 'datagrid');
		var opts = state.options;
		opts.onLoadSuccess.call(this, null);
	},
