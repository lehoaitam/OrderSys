...
$(document).ready(function() {
					...
					$('input[name="lastSelectedArea"]').each(function() {
						...
						if ($(this).attr('productIds') != '') {
							for (var i = 0; i < productIds.length; i++) {
                                var productName = '';
                                if (productNames.length > i) {
                                    productNames[i] = productNames[i].replace(/</g, '&lt;');
                                    productNames[i] = productNames[i].replace(/>/g, '&gt;');
                                    productName = productNames[i];
                                }
								if (checkProduct(productIds[i], productName)) {
									if (productTypes[i] == 'product') {
										content += '<li>' + productIds[i] + ':' + productName + '</li>';
									} else {
										content += '<li>' + productName + '</li>';
									}
								} else {
									if (productTypes[i] == 'product') {
										content += '<li style="color:red">' + productIds[i] + ':' + productName + '</li>';
									} else {
										var info = productIds[i].split(':');
										if (info[0] == 'goMenuSet') {
											deleteSelectedAreaById(group);
										}
										content += '<li style="color:red">' + (productName == "" ? productIds : productName) + '</li>';
									}
								}
							}
						}
    ...
    $(window).scroll(function(){
        if ($('.col-image').height() > $(".product-col").height()) {
            $(".product-col").stop().animate({"marginTop": 
            ($(window).scrollTop() > $('.col-image').offset().top ? 
                (parseInt($(window).scrollTop()) + parseInt($(".product-col").height()) > parseInt($('.col-image').offset().top) + parseInt($('.col-image').height()) ? 
                parseInt($('.col-image').height()) - parseInt($(".product-col").height()) : $(window).scrollTop() - $('.col-image').offset().top) : 0) + "px", 
                "marginLeft":($(window).scrollLeft()) + "px"}, "fast" );
        }
    });
});
